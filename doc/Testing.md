...
`GCOV_PREFIX` and `GCOV_PREFIX_STRIP` environment variables for each test case,
which causes the generated [GCDA][] coverage data files to be created under the
...
If the __`--geninfo`__ option is given in addition to `--coverage`, the test
framework will invoke [geninfo][] to generate one [lcov][] *tracefile* per test
case, named `coverage.info`:
...
Tracefiles may be merged together using the `lcov --add-tracefile` option, and
may also be combined into a single coverage report by passing many tracefile
arguments to the [genhtml][] utility.  Unfortunately, both of these operations
are prohibitively slow, which makes the `--geninfo` option of limited use for
the time being.
